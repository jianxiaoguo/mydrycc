<template>
    <div class="pv2 w-100 flex justify-end">
        ...
        <template v-for="page in pageList">
            <div @click="changePage(page)" class="cannotselect bl bb bt h3 w3 db tc b--light-gray pa1 f2 gray-70 "
                 :class="page===cPage?['bg-light-purple','white', 'cursor-not-allowed']:['cursor-pointer']">{{page}}</div>
        </template>
        <div v-if="hasNext" class="cannotselect bl bb bt h3 w3 db tc b--light-gray pa1 f2 gray-70 cursor-not-allowed">...</div>
        <div @click="addPage" class="cannotselect br--right ba br2 h3 w3 db pa1 b--light-gray cannorselect"
